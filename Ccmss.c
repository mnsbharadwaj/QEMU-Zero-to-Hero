...
int main() {
    ...
    // Perform CCM using one-shot function
    err = ccm_memory(
        cipher_idx,     // Cipher index
        key,            // Key
        sizeof(key),    // Key length
        NULL,           // PRNG state (unused)
        nonce,          // Nonce
        noncelen,       // Nonce length
        header,         // Additional auth data
        headerlen,      // Auth data length
        pt,             // Plaintext
        ptlen,          // Plaintext length
        ct,             // Ciphertext output
        tag,            // Tag output
        &taglen,        // Tag length
        CCM_ENCRYPT     // Direction
    );
    
    if (err != CRYPT_OK) 
        handle_error("ccm_memory failed", err);
    ...
    print_hex("Tag        ", expected_tag, taglen);
